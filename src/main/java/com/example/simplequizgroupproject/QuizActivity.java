...
public class QuizActivity extends AppCompatActivity {
    ...
    private void checkAnswer() {
        ...
        if (answNum == currentQuestion.getAnswerNum()) {
            score++; //generate a text view for the score
        } else {
            failed++;
        }
    }
    ...
    private void showNextQuestion() {
        group.clearCheck(); //unselects the radio buttons
        if(questioncounter < questioncounttotal){
            ...
            questioncounter++;
            textViewQuestion.setText("Question: " + questioncounter);
            answered = false; //checks if any question is answered
        } else {
            openResults();
        }
    }
    ...
    private void sendTextMessage() {
        try {
            ...
            Toast.makeText(getApplicationContext(), "Sms has failed, please try again", Toast.LENGTH_SHORT).show();
            e.printStackTrace();
        }
    }
}